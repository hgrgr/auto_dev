...
4. 🚨 [매우 중요: 초기 더미 데이터(Seed Data) 설계 필수]: E2E 테스터가 빈 화면 때문에 테스트에 실패하지 않도록, DB 스키마나 초기 상태(메모리 리스트 등)를 설계할 때 **API 명세서(Contract)의 응답 예시를 활용해 2~3개의 초기 더미 데이터(Seed Data)가 서버 시작 시 기본적으로 적재되도록 강력하게 명시**하세요. 빈 배열([]) 상태로 서버가 켜지는 설계는 절대 금지합니다.
